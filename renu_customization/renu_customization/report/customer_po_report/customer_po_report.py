...
#         conditions += " AND so.customer = %(customer_code)s"
 
#     if filters.get("customer_name"):
#         conditions += " AND so.customer_name LIKE %(customer_name)s"
 
#     if filters.get("item_code"):
#         conditions += " AND soi.item_code LIKE %(item_code)s"
...
#             so.transaction_date AS creation_date,
 
#             ROW_NUMBER() OVER (ORDER BY so.transaction_date ASC, so.name ASC) AS sr_no,
