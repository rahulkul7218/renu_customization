...
def get_dashboard_data(filters=None):
    ...
    if filters.get("business_region_name") and filters.get("business_region_name") != "All":
        gle_conditions.append(f"EXISTS (SELECT 1 FROM `tabCustomer` cust WHERE cust.name = gle.party AND cust.business_region_name = {frappe.db.escape(str(filters.get('business_region_name')))})")
    ...
    results_query = f"""
        SELECT
            ...
            cust.customer_group, cust.business_region_name,
    ...
    if filters.get("business_region_name") and filters.get("business_region_name") != "All":
        due_si_cond.append(f"EXISTS (SELECT 1 FROM `tabCustomer` cust WHERE cust.name = si.customer AND cust.business_region_name = {frappe.db.escape(str(filters.get('business_region_name')))})")

    due_si = frappe.db.sql(f"SELECT 'Sales Invoice' as doctype, si.name, si.customer, si.customer_group, cust.business_region_name, si.posting_date, si.due_date, si.outstanding_amount, si.base_grand_total as base_net_total, DATEDIFF(si.due_date, CURDATE()) as due_days FROM `tabSales Invoice` si LEFT JOIN `tabCustomer` cust ON cust.name = si.customer WHERE {' AND '.join(due_si_cond)}", as_dict=True)
...
def export_to_excel(filters=None, export_type="all"):
    ...
    if export_type in ["all", "detail"]:
        ...
        headers = ["S.No.", "Voucher No", "Voucher Type", "Reference", "Date", "Due Date", "Days Diff", "Customer", "Customer Group", "Business Region", "Sales Person", "Amount (M)", "Status"]
        ...
        for r_idx, row in enumerate(results):
            ...
            cells = [
                ...
                ws_list.cell(row=row_idx, column=10, value=row.get('business_region_name') or "-"),
                ws_list.cell(row=row_idx, column=11, value=row.get('sales_person')),
                ws_list.cell(row=row_idx, column=12, value=flt(row.get('allocated_amount')) / 1000000),
                ws_list.cell(row=row_idx, column=13, value=row.get('status'))
            ]
        ...
        total_cell = ws_list.cell(row=row_idx, column=12, value=total_amt)
    ...
    if export_type in ["all", "due"]:
        ...
        headers = ["S.No.", "Invoice ID", "Customer", "Customer Group", "Business Region", "Sales Person", "Posting Date", "Due Date", "Due Days", "Net Total (M)", "Outstanding (M)"]
        for idx, h in enumerate(headers, start=1):
        ...
        for r_idx, row in enumerate(due_results):
            ...
            cells = [
                ...
                ws_due.cell(row=row_idx, column=5, value=row.get('business_region_name') or "-"),
                ws_due.cell(row=row_idx, column=6, value=row['sales_person']),
                ws_due.cell(row=row_idx, column=7, value=row['posting_date']),
                ws_due.cell(row=row_idx, column=8, value=row['due_date']),
                ws_due.cell(row=row_idx, column=9, value=row['due_days']),
                ws_due.cell(row=row_idx, column=10, value=flt(row['base_net_total']) / 1000000),
                ws_due.cell(row=row_idx, column=11, value=flt(row['outstanding_amount']) / 1000000)
            ]
            for c_idx, c in enumerate(cells, start=1):
                c.border = table_border
                if row_f: c.fill = row_f
        ...
        ws_due.merge_cells(start_row=row_idx, start_column=1, end_row=row_idx, end_column=9)
        for c in range(1, 12):
            ws_due.cell(row=row_idx, column=c).fill = header_fill
            ws_due.cell(row=row_idx, column=c).border = table_border

        ws_due.cell(row=row_idx, column=10, value=total_due_net).font = header_font
        ...
        ws_due.cell(row=row_idx, column=11, value=total_due_out).font = header_font
        ws_due.cell(row=row_idx, column=11).number_format = num_format
        ws_due.cell(row=row_idx, column=11).alignment = Alignment(horizontal="right")

    # Column Widths
    for ws in wb.worksheets:
        max_col = 13 if ws.title == "Detailed Collection List" else 11
