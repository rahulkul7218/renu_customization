                        ...
                        try {
                            ...
                            workbook.SheetNames.forEach(sheetName => {
                                ...
                                for (let idx = start_idx; idx < rows.length; idx++) {
                                    ...
                                    // Merge repeated item codes
                                    if (!serial_map[item_code]) {
                                        serial_map[item_code] = { serials: [], uom: uom };
                                    }
                                    serial_map[item_code].serials.push(...serials);
                                }
                            });
                            ...
                            // --- Distribute serials across existing lines ---
                            Object.keys(serial_map).forEach(item_code => {
                                let uploaded_serials = serial_map[item_code].serials;
                                let serial_idx = 0;

                                // Find all existing rows for this item
                                let existing_rows = frm.doc.items.filter(d => d.item_code === item_code);

                                if (existing_rows.length === 0) {
                                    // If no existing row, create a new one with all serials
                                    let item_row = frm.add_child("items");
                                    let final_qty = uploaded_serials.length;
                                    ...
                                    existing_rows.forEach(row => {
                                        ...
                                        let assigned_serials = uploaded_serials.slice(serial_idx, serial_idx + row_qty);

                                        if (assigned_serials.length < row_qty) {
                                            frappe.throw(`Not enough serial numbers for item ${item_code} for quantity ${row_qty}`);
                                        }

                                        row.serial_no = assigned_serials.join("\n");
                                        row.received_qty = row_qty;
                                        row.accepted_qty = row_qty;
                                        row.rejected_qty = 0;
                                    ...
                                    if (serial_idx < uploaded_serials.length) {
                                        let remaining_serials = uploaded_serials.slice(serial_idx);
                                        let item_row = frm.add_child("items");
                                        let final_qty = remaining_serials.length;
